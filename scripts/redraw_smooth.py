...
existing =False   
while line:           
    if line[0]=="#" and line[1:-1]==redraw_name:
        existing= True
        break
    line = f.readline()
if existing==False:
    print(f"{redraw_name} is not existing.")
    exit(0)
line = f.readline()    
while line:              
    if line[0]=="#":
        break
    local_path.append(line[:-1])
    line = f.readline()
idx = redraw_name.find("-")
case_name = redraw_name[:idx]
unique_idx = int(redraw_name[idx+5:])
